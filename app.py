...
""", unsafe_allow_html=True)
    ...
    # Upload CPU File
    uploaded_line = st.file_uploader("Upload Line cards File", type=["xlsx"], key="cpu")
    if uploaded_line:
    ...
    if st.session_state.get("line_data") is not None:
        try:
            ...
            df_merged = pd.merge(
                df_cpu,
                df_ref[["Site Name", "Call ID","Mapping","Instant BER After FEC","Threshold",
            ...
            if not df_merged.empty:
                df_result = df_merged[[
                    "Site Name", "ME","Call ID", "Measure Object","Threshold",
                    "Instant BER After FEC","Maximum threshold(Out)", "Minimum threshold(Out)","Output Optical Power (dBm)",
                    "Maximum threshold(In)", "Minimum threshold(In)","Input Optical Power (dBm)"
                ]]
                ...
                # ฟังก์ชันตรวจไม่ผ่าน
                def is_not_ok(row):
                    return row["Output Optical Power (dBm)"] > row["Maximum threshold(Out)"] or row["Output Optical Power (dBm)"] < row["Minimum threshold(Out)"] or 
